...
local net = local_configs.enable_net
    and lain.widget.net({
      settings = function()
        widget:set_markup(
...
local function buildRightWidgets(s, wp)
  ...
  return {
    ...
    local_configs.enable_bat and wibox.container.background(
      wibox.container.margin(
    ...
    local_configs.enable_net and wibox.container.background(
      wibox.container.margin(
        wibox.widget({ nil, nil, net and net.widget, layout = wibox.layout.align.horizontal }),
        dpi(3, s),
        dpi(3, s)
      ),
      "#5e3636"
...
function theme.at_screen_connect(s)
  ...
  if local_configs.screen_dpi then
    xresources.set_dpi(
      s.dpi >= local_configs.screen_dpi.high and local_configs.screen_dpi.high or local_configs.screen_dpi.low,
      s
    )
  end

  gears.debug.print_warning(
    string.format("Setting Up Screen %s %sx%s DPI: %s", s.index, s.geometry.width, s.geometry.height, s.dpi)
  )
  ...
  screen.connect_signal("property::geometry", function(signal_screen)
    ...
    if s.index == signal_screen.index then
      if is_horizon ~= (s.geometry.width >= s.geometry.height) then
        ...
        gears.debug.print_warning(
          string.format("Changing wallpaper path by resolution change: %sx%s", s.geometry.width, s.geometry.height)
        )
        wp.change_path(wallpaper_path_for(nil))
      end
    else
      if wp.current then
        wp.set_wallpaper(wp.current)
      end
    end
  end)
